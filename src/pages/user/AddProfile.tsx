...
import { AddProfileFormData, useAddProfleValidate } from "../../utils/formValidation/AddProfileFormData";
...
const AddProfile = () => {
    ...
    const handleInputChange = (e: ChangeEvent<HTMLInputElement>) => {
      ...
      if (inputValue.length <= 2) {
        ...
        setDob(`${inputValue.slice(0, 2)}/${inputValue.slice(2, 4)}/${inputValue.slice(4, 8)}`);
      }
    };
  ...
  <div className="relative flex justify-center md:items-center">
    <div className="relative bg-amber-50 shadow-xl overflow-hidden flex justify-center ring-1 w-full lg:w-[70vw] h-[85vh] md:h-[80vh] ring-gray-900/5 rounded-3xl sm:w-full sm:rounded-xl sm:px-10">
      <form onSubmit={handleSubmit(formSubmit)} className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-10 grid-rows-12 gap-5">
  ...
  <textarea
    className="p-2 outline-none border w-full md:w-70 lg:w-72 border-amber-150 h-28 md:h-36 rounded-md text-teal-800 placeholder:font-thin placeholder:text-zinc-300 placeholder:text-sm"
    placeholder="abc"
    typeof="text"
    cols={5}
    defaultValue={""}
        ...
        <div className="ml-12 md:ml-0 lg:ml-10 col-span-full sm:col-span-2 md:col-span-5 row-start-9 md:row-start-10 mt-9">
          <input type="radio" /> &nbsp;
          <label htmlFor="" className="text-teal-800">male</label> &nbsp;
          <input type="radio" />&nbsp;
          <label htmlFor="" className="text-teal-800">female</label>&nbsp;
        </div>
