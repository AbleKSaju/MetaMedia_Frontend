...
const PostScroll = ({data,render,setRender}:any) => {
  ...
  const like = async () => {
    ...
    if (responce.status) {
      if(responce.data.likes.includes(data?.userData?.basicInformation?.userId)){
  ...
  return (
    ...
    {console.log(data,'this si daatttaa')
      ...
      <div className=" md:p-5  sm:p-3 p-7 w-screen sm:w-full flex justify-center  sm:pl-0   lg:p-4   ">
        <div className=" flex  flex-col justify-center  bg-white shadow-md bg-clip-border rounded-lg lg:w-[464px] lg:h-[800px] border w-[300px] md:w-[450px] sm:w-[330px]">
          ...
          <div className="flex p-2 sm:p-4 pl-5 md:p-3 sm:pl-5 ">
            <img
              ...
              src={`http://localhost:3000/profile/${data.userData?.profile?.profileUrl}`}
              alt=""
            />
          ...
          <div className="relative mx-4 mt-0 overflow-hidden text-gray-700  shadow-lg bg-clip-border  rounded-md lg:h-[500px]" onClick={()=>handlePostClick(data)}>
            <img
          ...
          <div onClick={like}>
            ...
            <div>
              {" "}
              <MessageCircle className="text-[#07312E] " onClick={()=>handlePostClick(data)}/>
            </div>
            <div>
              {" "}
        ...
        <p onClick={()=>handlePostClick(data)}>View {data.comments.length} comments</p>
      ) : (
        <p>Add a comment</p>
      )}
          </div>
         <div className="ml-9 text-[12px] text-gray-500"> {timeAgo.format(new Date(data.createdAt))}</div>
