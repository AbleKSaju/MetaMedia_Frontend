...
import {Home,Search,LucideMessageSquareText,Clapperboard,ImagePlus,BellRing} from 'lucide-react'
...
const Sidebar = () => {
  ...
  return (
    <>
      ...
      >
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={`absolute cursor-pointer -right-0 text-amber-50 top-9 w-7 mr-1 mt-[750px]
            ${!open && "rotate-180"}`}
...
<div className="flex gap-x-4 items-center">
      <motion.div
         whileHover={{ scale:0.7,transition:{ease:'easeInOut',duration:1.2} }}
  ...
  className={`flex mt-5 rounded-md p-2 cursor-pointer hover:bg-teal-700 hover:rounded-xl text-amber-50 text-xl font-sans font-semibold items-center gap-x-4 
   ${0 === 0 && "bg-light-white"} ${selectedMenu === 0 && "bg-teal-700 rounded-xl "}`}
  onClick={() => HandleSidebarClick(0)}
>
<motion.div

  whileHover={{ scale: 1.6, rotate: 360,transition:{ease:'easeInOut',duration:0.9} }}
...
>
    {selectedMenu === 0 ? (
     <LucideMessageSquareText  className={!open && "hidden" || ''}/>
...
>
    ...
    >
           <motion.div
  whileHover={{ scale: 1.6 ,transition:{ease:'easeInOut',duration:0.9}}}
  whileTap={{
    scale: 1.7,
    rotate: 360,
...
>
    {selectedMenu === 0 ? (
      <img className="w-7 h-7 rounded-full" src="https://i.pinimg.com/564x/53/12/21/531221133b6028752e2a523b6d8cdc5d.jpg" />
